...
class clusterSettingDialog(wx.Dialog):
    ...
    def __init__(self, cluster_address, tasks_per_node, work_dir, setup_script, run_command):
        ...
        cluster_address_label = wx.StaticText(
            self.panel, label="Cluster Address:", size=(100, -1))
        cluster_address_label.SetToolTip(wx.ToolTip(
            "The URL of the cluster. If logged in, changing this will cause you to logout."
        ))
        cluster_address_sizer.Add(
            cluster_address_label, 0, wx.ALL | wx.CENTER, 5, )
        self.cluster_address = wx.TextCtrl(
            self.panel, style=wx.TE_PROCESS_ENTER, value=cluster_address, size=(300, -1))
        ...
        tasks_per_node_label = wx.StaticText(
            self.panel, label="Tasks Per Node:", size=(300, -1))
        ...
        tasks_per_node_sizer.Add(tasks_per_node_label,
                                 0, wx.ALL | wx.CENTER, 5)
        self.tasks_per_node = wx.SpinCtrl(
            self.panel, value=str(tasks_per_node), size=(100, -1))
        ...
        max_runtime_label = wx.StaticText(
            self.panel, label="Runtime limit (hours):", size=(300, -1))
        ...
        self.max_runtime = wx.SpinCtrl(
            self.panel, value=str(max_runtime), size=(100, -1))
        max_runtime_sizer.Add(self.max_runtime, 0, wx.ALL, 5)

        # work_dir field
        work_dir_sizer = wx.BoxSizer(wx.HORIZONTAL)
        work_dir_label = wx.StaticText(
            self.panel, label="Working Directory:", size=(100, -1))
        ...
        self.work_dir = wx.TextCtrl(
            self.panel, style=wx.TE_PROCESS_ENTER, value=work_dir, size=(300, -1))
        work_dir_sizer.Add(self.work_dir, 0, wx.ALL, 5)

        # setup_script field
        setup_script_label_sizer = wx.BoxSizer(wx.HORIZONTAL)
        setup_script_label = wx.StaticText(
            self.panel, label="Setup Script:", size=(100, -1))
        ...
        setup_script_label_sizer.Add(
            setup_script_label, 0, wx.ALL | wx.CENTER, 5)

        setup_script_field_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.setup_script = wx.TextCtrl(
            self.panel, value=setup_script, size=(400, 80), style=wx.TE_MULTILINE)
        setup_script_field_sizer.Add(self.setup_script, 0, wx.ALL, 5)

        # run_command field
        run_command_label_sizer = wx.BoxSizer(wx.HORIZONTAL)
        run_command_label = wx.StaticText(
            self.panel, label="Run Command:", size=(100, -1))
        ...
        run_command_label_sizer.Add(
            run_command_label, 0, wx.ALL | wx.CENTER, 5)

        run_command_field_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.run_command = wx.TextCtrl(
            self.panel, value=run_command, size=(400, 80), style=wx.TE_MULTILINE)
        run_command_field_sizer.Add(self.run_command, 0, wx.ALL, 5)

        # The Ok and Cancel button
        button_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.ok_button = wx.Button(
            self.panel, wx.ID_OK, label="Ok", size=(60, 30))
        button_sizer.Add(self.ok_button, 0, wx.ALL, 5)

        self.cancel_btn = wx.Button(
            self.panel, wx.ID_CANCEL, label="Cancel", size=(60, 30))
        button_sizer.Add(self.cancel_btn, 0, wx.ALL, 5)

        # Bind enter press to the Ok button
        button_event = wx.PyCommandEvent(
            wx.EVT_BUTTON.typeId, self.ok_button.GetId())
        self.cluster_address.Bind(
            wx.EVT_TEXT_ENTER, lambda e: wx.PostEvent(self, button_event))
        self.tasks_per_node.Bind(
            wx.EVT_TEXT_ENTER, lambda e: wx.PostEvent(self, button_event))
        self.max_runtime.Bind(
            wx.EVT_TEXT_ENTER, lambda e: wx.PostEvent(self, button_event))
        self.max_runtime.Bind(
            wx.EVT_TEXT_ENTER, lambda e: wx.PostEvent(self, button_event))
...
class LoginDialog(wx.Dialog):
    ...
    def __init__(self, username=''):
        ...
        self.username = wx.TextCtrl(self.panel, value=username, size=(
            160, -1), style=wx.TE_PROCESS_ENTER)
        ...
        self.password = wx.TextCtrl(self.panel, size=(
            160, -1), style=wx.TE_PASSWORD | wx.TE_PROCESS_ENTER)
        password_sizer.Add(self.password, 0, wx.ALL, 5)

        # The login and cancel button
        button_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.ok_button = wx.Button(
            self.panel, wx.ID_OK, label="Login", size=(60, 30))
        button_sizer.Add(self.ok_button, 0, wx.ALL, 5)
        ...
        self.settings_button = wx.Button(
            self.panel, wx.ID_PREFERENCES, label="Settings", size=(60, 30))
...
def update_cluster_parameters():
    ...
    dialog = clusterSettingDialog(
        cluster_address, tasks_per_node, work_dir, setup_script, run_command)
    result = dialog.ShowModal()
    if result == wx.ID_OK:
