...
class ClusterSettingDialog(wx.Dialog):
    ...
    def __init__(self, cluster_address, tasks_per_node, work_dir, setup_script):
        ...
        cluster_address_label = wx.StaticText(self.panel,
                                              label="Cluster Address:",
                                              size=(100, -1))
        msg = (f"The URL of the cluster. If logged in, changing this will cause"
               f" you to logout.")
        cluster_address_label.SetToolTip(wx.ToolTip(msg))
        ...
        tasks_per_node_label = wx.StaticText(self.panel,
                                             label="Tasks Per Node:",
                                             size=(300, -1))
        msg = (f"The number of individual processes that can be run on a single"
               f" node. Usually the number of cpu core per node.")
        tasks_per_node_label.SetToolTip(wx.ToolTip(msg))
        tasks_per_node_sizer.Add(tasks_per_node_label, 0, wx.ALL | wx.CENTER, 5)
        self.tasks_per_node = wx.SpinCtrl(self.panel, value=str(tasks_per_node),
                                          size=(100, -1))
        ...
        max_runtime_label = wx.StaticText(self.panel,
                                          label="Runtime limit (hours):",
                                          size=(300, -1))
        msg = (f"The maximum runtime limit on the cluster. You can set a lower"
               f" limit for individual jobs.")
        max_runtime_label.SetToolTip(wx.ToolTip(msg))
        max_runtime_sizer.Add(max_runtime_label, 0, wx.ALL | wx.CENTER, 5)
        self.max_runtime = wx.SpinCtrl(self.panel, value=str(max_runtime),
                                       size=(100, -1))
        ...
        work_dir_label = wx.StaticText(self.panel, label="Working Directory:",
                                       size=(100, -1))
        msg = (f"Path to the directory in which the job is run. The "
               f"{{username}} tag is replaced by your username on the cluster "
               f"and is useful when multiple people use the same local "
               f"machine.")
        ...
        setup_script_label = wx.StaticText(self.panel, label="Setup Script:",
                                           size=(100, -1))
        msg = (f"Run at the start of each job. Used for setting up the run, for"
               f" example loading the CellProfiler module. Can be any general "
               f"bash script, but may be limited by the cluster. For example, "
               f"compute nodes often lack internet access and cannot install "
               f"python packages.")
        setup_script_label.SetToolTip(wx.ToolTip(msg))
        setup_script_label_sizer.Add(setup_script_label, 0, wx.ALL | wx.CENTER,
                                     5)

        setup_script_field_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.setup_script = wx.TextCtrl(self.panel, value=setup_script,
                                        size=(400, 80), style=wx.TE_MULTILINE)
        ...
        button_event = wx.PyCommandEvent(wx.EVT_BUTTON.typeId,
                                         self.ok_button.GetId())
        self.cluster_address.Bind(wx.EVT_TEXT_ENTER,
                                  lambda e: wx.PostEvent(self, button_event))
        self.tasks_per_node.Bind(wx.EVT_TEXT_ENTER,
                                 lambda e: wx.PostEvent(self, button_event))
        self.max_runtime.Bind(wx.EVT_TEXT_ENTER,
                              lambda e: wx.PostEvent(self, button_event))
        self.max_runtime.Bind(wx.EVT_TEXT_ENTER,
                              lambda e: wx.PostEvent(self, button_event))
        self.work_dir.Bind(wx.EVT_TEXT_ENTER,
                           lambda e: wx.PostEvent(self, button_event))
...
class LoginDialog(wx.Dialog):
    ...
    def __init__(self, username=''):
        ...
        self.password = wx.TextCtrl(self.panel, size=(160, -1),
                                    style=wx.TE_PASSWORD | wx.TE_PROCESS_ENTER)
        password_sizer.Add(self.password, 0, wx.ALL, 5)

        # The login and cancel button 
        button_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.ok_button = wx.Button(self.panel, wx.ID_OK, label="Login",
                                   size=(60, 30))
        button_sizer.Add(self.ok_button, 0, wx.ALL, 5)
        ...
        self.settings_button = wx.Button(self.panel, wx.ID_PREFERENCES,
                                         label="Settings", size=(60, 30))
        button_sizer.Add(self.settings_button, 0, wx.ALL, 5)

        # Bind enter press to the Login button
        button_event = wx.PyCommandEvent(wx.EVT_BUTTON.typeId,
                                         self.ok_button.GetId())
        self.username.Bind(wx.EVT_TEXT_ENTER,
                           lambda e: wx.PostEvent(self, button_event))
        self.password.Bind(wx.EVT_TEXT_ENTER,
                           lambda e: wx.PostEvent(self, button_event))
...
def update_cluster_parameters():
    ...
    dialog = ClusterSettingDialog(cluster_address, tasks_per_node, work_dir,
                                  setup_script)
    result = dialog.ShowModal()
    if result == wx.ID_OK:
...
def CPRynner():
    ...
    if cprynner is None:
        try:
            ...
            msg = (f"Unable to contact the cluster. The cluster may be offline "
                   f"or you may have a problem with your internet connection.")
