...
from CPRynner.CPRynner import (CPRynner, cluster_max_runtime,
                               cluster_run_command, cluster_setup_script,
...
class RunOnCluster(Module):
    ...
    def create_settings(self):
        ...
        doc_ = (
            f"Enter a recognisable identifier for the run "
            f"(spaces will be replaced by undescores)"
        )
        self.runname = Text("Run Name", "Run_name", doc=doc_)

        doc_ = (
            f"The number of image files in each measurement that must be "
        ...
        doc_ = (
            f"The maximum time for reserving a node on the cluster. Should"
        ...
        doc_ = (
            f"Enter a project code of an Supercomputing Wales project you "
        ...
        doc_ = (
            f"Select the partition you wish to run your job on. This may "
        ...
        doc_ = (
            f"Choose where local copies of remote scripts and batch data are" f" saved."
        )
    ...
    @staticmethod
    def group_images(list_, n_measurements, measurements_per_run, groups_first=True):
        ...
        if groups_first:
            ...
            return [(int(i / images_per_run), name) for i, name in enumerate(list_)]
        else:
            return [
                (int((i % n_measurements) / measurements_per_run), name)
    ...
    def assign_image_groups(self, file_list):
        ...
        if not self.is_archive.value:
            ...
            grouped_images = self.group_images(
                file_list, n_measurements, measurements_per_run, self.type_first.value
            )
            image_group_list = list(zip(*grouped_images))
            n_image_groups = max(image_group_list[0]) + 1
            # Add image files to uploads
            uploads = [[name, f"run{g}/images"] for g, name in grouped_images]
        else:
            if n_images > 1:
                wx.MessageBox(
    ...
    def create_run_scripts(
        self, workspace, rynner, uploads, n_image_groups, grouped_images
    ):
        max_tasks = int(cluster_tasks_per_node())
        ...
        for g in range(n_image_groups):
            ...
            if not self.is_archive.value:
                n_measurements = int(
                    len([i for i in grouped_images if i[0] == g])
                    / self.n_images_per_measurement.value
                )
                script = (
                    f"{run_command} -p Batch_data.h5 -o "
    ...
    def prepare_run(self, workspace):
        ...
        if self.batch_mode.value:
            ...
            if rynner is not None:
                ...
                if (
                    (uploads is None)
                    or (grouped_images is None)
                    or (n_image_groups is None)
                ):
                    return False
                ...
                downloads = [[f"run{g}", output_dir] for g in range(n_image_groups)]
                ...
                dialog = wx.GenericProgressDialog(
                    "Uploading", "Uploading files", style=wx.PD_APP_MODAL
                )
    ...
    def validate_module_warnings(self, pipeline):
        ...
        if pipeline.test_mode:
            raise ValidationError(
                "RunOnCluster will not produce output in Test Mode", self.runname
            )
    ...
    def save_remote_pipeline(self, workspace, outf=None):
        ...
        try:
            ...
            for module in reversed(pipeline.modules()):
                if (
                    module.module_name == "RunOnCluster"
                    or module.module_name == "ClusterView"
                ):
